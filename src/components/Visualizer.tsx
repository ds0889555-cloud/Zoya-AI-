...
type VisualizerState = "idle" | "listening" | "processing" | "speaking" | "researching";
...
export default function Visualizer({ state }: VisualizerProps) {
  const getRingAnimation = (index: number, reverse: boolean = false) => {
    const baseSpeed = state === "listening" ? 3 : (state === "processing" || state === "researching") ? 1.5 : state === "speaking" ? 2 : 15;
    return {
  ...
  const getPulseAnimation = () => {
    ...
    if (state === "processing" || state === "researching") {
      return {
  ...
  const getTheme = () => {
    switch (state) {
      ...
      case "researching": return { color: "rgba(34, 211, 238, 1)", glow: "shadow-cyan-400/80", border: "border-cyan-400" };
